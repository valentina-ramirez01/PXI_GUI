...
class BandwidthGUI:
    ...
    def build_gui(self):
        ...
        note = ttk.Label(
            settings,
            text="CH0 = IN+, CH1 = OUT. Gain = RMS(CH1) / RMS(CH0). Use gain-11 wiring: OUT→10.02kΩ→IN−, IN−→1kΩ→GND.",
            foreground="blue"
        )
        ...
        self.table.heading("gain_db", text="Gain [dB]")
    ...
    def reset_plot(self):
        self.ax.clear()
        self.ax.set_title("OPA551 Stepped Sine Bandwidth")
        self.ax.set_xlabel("Frequency [Hz]")
        self.ax.set_ylabel("Gain [dB]")
        self.ax.set_xscale("log")
        self.ax.grid(True, which="both")
        self.ax.axhline(-3, linestyle="--", label="-3 dB from low-frequency gain")
        self.ax.legend()
        self.canvas.draw()
        ...
        try:
            ...
            for f in freqs:
                ...
                for _ in range(n_avg):
                    ...
                    win = scope.channels["0"].fetch(
                        num_samples=sample_count,
                        timeout=10.0
                    )[0]

                    wout = scope.channels["1"].fetch(
                        num_samples=sample_count,
                        timeout=10.0
                    )[0]

                    vin = np.array(win.samples, dtype=float)
                    vout = np.array(wout.samples, dtype=float)
                    ...
                    vin = vin - np.mean(vin)
                    vout = vout - np.mean(vout)

                    vin_rms = np.sqrt(np.mean(vin ** 2))
                    vout_rms = np.sqrt(np.mean(vout ** 2))

                    if vin_rms > 0:
                        gain_vals.append(vout_rms / vin_rms)

                gain_at_f = np.mean(gain_vals)
                gains.append(gain_at_f)

            gains = np.array(gains)
            gain_db = 20 * np.log10(gains)

            reference_gain_db = gain_db[0]
            target_db = reference_gain_db - 3.0
            ...
            if bw is not None:
                result = "-3 dB Bandwidth ≈ {:.3f} MHz | Low-Frequency Gain ≈ {:.2f} dB".format(
                    bw / 1e6,
                    reference_gain_db
                )
            else:
                result = "No -3 dB point found | Low-Frequency Gain ≈ {:.2f} dB".format(
                    reference_gain_db
                )
    ...
    def update_plot(self):
        ...
        self.ax.semilogx(self.freqs, self.gain_db, marker="o", label="Measured Gain")

        target_db = getattr(self, "target_db", None)

        if target_db is not None:
            self.ax.axhline(target_db, linestyle="--", label="-3 dB Level")
