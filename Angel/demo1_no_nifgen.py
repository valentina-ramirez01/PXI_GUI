...
# ============================================================
# HOST / DEMO MODE
# This lets the Dash GUI run on a host machine that does NOT have
# NI-FGEN, NI drivers, or PXIe hardware connected.
#
# The GUI still shows the waveform preview using the existing
# synth_standard_chunk() function. The mock session only replaces
# the hardware calls made when you press Configure & Start / Stop.
#
# When you move back to the PXIe host, set USE_MOCK_FGEN=0 in the
# environment OR change DEFAULT_USE_MOCK_FGEN to False.
# ============================================================
DEFAULT_USE_MOCK_FGEN = True
USE_MOCK_FGEN = os.getenv("USE_MOCK_FGEN", "1" if DEFAULT_USE_MOCK_FGEN else "0") == "1"

if not USE_MOCK_FGEN:
    import nifgen
else:
    class MockWaveform:
        SINE = "SINE"
    ...
    class MockSession:
        def __init__(self, resource_name):
            ...
            print(f"[MOCK FGEN] Created mock session for {resource_name}")

        def abort(self):
            print("[MOCK FGEN] abort() called")

        def configure_standard_waveform(self, waveform, amplitude, frequency, dc_offset, start_phase):
            print(
                f"[MOCK FGEN] configure_standard_waveform: "
                f"waveform={waveform}, frequency={frequency} Hz, "
                f"amplitude={amplitude} Vpp, dc_offset={dc_offset}, phase={start_phase}"
            )

        def initiate(self):
            print("[MOCK FGEN] initiate() called")
...
if __name__ == "__main__":
    # host="0.0.0.0" is useful when running inside a container/remote host.
